...
public class LibraryConfiguration {
    ...
    /// <summary>
    /// The virtual root for the library.
    /// </summary>
    [XmlIgnore]
    [JsonIgnore]
    public string VirtualRoot => string.IsNullOrWhiteSpace(CustomVirtualRoot)
        ? Path.Join(Plugin.Instance.VirtualRoot, Id.ToString())
        : Path.Combine(Plugin.Instance.VirtualRoot, CustomVirtualRoot);

    /// <summary>
    /// The user-defined virtual root for the library, if specified.
    /// </summary>
    /// <remarks>
    /// Specified as an absolute path, or a path relative to the plugin's
    /// virtual root.
    /// </remarks>
    public string? CustomVirtualRoot { get; set; }
    ...
    /// <summary>
    /// The media folders associated with this library.
    /// </summary>
    public IReadOnlyList<MediaFolderConfiguration> MediaFolders => [.. Plugin.Instance.Configuration.LibraryFolders.Where(config => config.LibraryId == Id)];
}
