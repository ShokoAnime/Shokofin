...
public class ExtraMetadataProvider : IServerEntryPoint
{
    ...
    private void UpdateSeason(Season season, Series series, string seriesId, bool deleted = false)
    {
        ...
        if (seasonNumber == 0) {
            if (deleted && AddVirtualSeason(0, series) is Season virtualSeason) 
                season = virtualSeason;
        ...
        // Every other "season".
            ...
            if (deleted && AddVirtualSeason(seasonInfo, offset, seasonNumber, series) is Season virtualSeason)
                season = virtualSeason;
    ...
    private void UpdateEpisode(Episode episode, string episodeId)
    {
        ...
        var seasonInfo = ApiManager.GetSeasonInfoForEpisode(episodeId)
            ...
            .GetResult();
        if (seasonInfo == null) {
            Logger.LogWarning("Unable to find season info for episode. (Episode={EpisodeId})", episode);
            return;
        }
        var episodeInfo = seasonInfo.EpisodeList.FirstOrDefault(e => e.Id == episodeId);
        if (episodeInfo == null) {
            Logger.LogWarning("Unable to find episode info for episode. (Episode={EpisodeId})", episode);
            return;
        }
